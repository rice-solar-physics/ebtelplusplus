...
def run_example():
    ...
    # Time average DEM
    results_dem_total = np.average(
        results['dem_tr']+results['dem_corona'],
        axis=0,
        weights=np.gradient(results['dem_temperature']))
    results_dem_tr = np.average(
        results['dem_tr'],
        axis=0,
        weights=np.gradient(results['dem_temperature']))
    results_dem_corona = np.average(
        results['dem_corona'],
        axis=0,
        weights=np.gradient(results['dem_temperature']))
    ...
    ax4.plot(results['dem_temperature'], results_dem_tr,
             color=seaborn.color_palette()[0],
             label=r'$\mathrm{DEM}_{\mathrm{TR}}$')
    ax4.plot(results['dem_temperature'], results_dem_corona,
             color=seaborn.color_palette()[2],
             label=r'$\mathrm{DEM}_{\mathrm{C}}$')
    ax4.plot(results['dem_temperature'], results_dem_total,
    ...
    plt.savefig(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), 'ex1.png'))
