...
for i in range(len(alpha)):
    ...
    #build surface plot
    surf_plot.plot_surface(Tn,loop_length,temp_max_save,xlab=r'$L$ (Mm)',ylab=r'$T_n$ (s)',plot_title=r'$T_{max}$ Surface, $\alpha=$'+str(alpha[i]),print_fig_filename=root_dir+figdir%(args.species,alpha[i])+'t_max_surface_'+args.species+'_alpha'+str(alpha[i])+'_tpulse'+str(tpulse)+'_'+solver)
    surf_plot.plot_surface(Tn,loop_length,em_max_save,xlab=r'$L$ (Mm)',ylab=r'$T_n$ (s)',plot_title=r'EM$_{max}$ Surface, $\alpha=$'+str(alpha[i]),print_fig_filename=root_dir+figdir%(args.species,alpha[i])+'em_max_surface_'+args.species+'_alpha'+str(alpha[i])+'_tpulse'+str(tpulse)+'_'+solver)
